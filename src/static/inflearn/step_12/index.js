...
/** =======================================
 * C.map, C.filter
 * 특정 함수 라인에서만 병렬적으로 평가하고 그 이후는
 * 동기적 실행.
======================================= */
// ===== 한번에 실행한다.
// C.map(a => delay1000(a * a), [1, 2, 3, 4]).then(a => log('> C.map : ', a));
// C.filter(a => delay1000(a * 2), [1, 2, 3, 4]).then(a => log('> C.filter : ', a));
...
// console.time('L');
// go([1, 2, 3, 4, 5, 6, 7, 8],
//   L.map(a => delay500(a * a, 'L.map 1')),
//   L.filter(a => delay500(a % 2, 'L.filter 2')),
//   L.map(a => delay500(a + 1, 'L.map 3')),
//   take(2),
//   log,
//   _ => console.timeEnd('L')
//   );
...
// go([1, 2, 3, 4, 5, 6, 7, 8],
//   L.map(a => delay500(a * a, 'L.map 1')),
//   C.filter(a => delay500(a % 2, 'C.filter 2')),
//   L.map(a => delay500(a + 1, 'L.map 3')),
//   take(2),
//   log,
//   _ => console.timeEnd('C')
