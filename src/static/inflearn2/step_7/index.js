...
// ===== 배열을 출력시 콤마가 나오는 것을 막기 위해 문자열을 합쳐준다.
const string = iter => _.reduce((a, b) => `${a}${b}`, iter);
// # 사용 예
// ${string(L.map(img => ` <div class="name">${img.name}</div>`, imgs))}
...
// # 위의 qs와 qsa는 인자가 가변이라 curry를 적용할 수 없어 별도의 함수를 만든다.
$.find = _.curry($.qs);
$.findAll = _.curry($.qsa);
...
// # 현재 엘리먼트에서 상위 엘리먼트를 찾는다.
$.closest = _.curry((sel, el) => el.closest(sel));

// # 현재 엘리먼트 삭제
$.remove = (el) => el.parentNode.removeChild(el);

// # 엘리먼트에 이벤트를 바인딩 한다.(여러개 혹은 한개의 엘리먼트도 모두 적용.)
$.on = (event, f) => els =>
...
// ===== 랜더링
_.go(
  Images.fetch(), // 데이터 가져오기
  Images.tmpl, // 템플릿 생성
  $.el, // 엘리먼트 생성
  // el => document.querySelector('body').appendChild(el), //  body에 붙이기
  $.append($.qs('body')), // body에 붙이기. 위의 코드 간편하게
  $.findAll('.image'), // 전체 이미지를 찾음.
  $.on('click',e => { // 이미지에 이미지 삭제 이벤트 등록
    _.go(
      e.currentTarget, // 현재 타겟
      $.closest('.image'), // 현재 타겟 의 image 엘리먼트
      $.remove, // image 엘리먼트 삭제
    )
  }),
  console.log
)
